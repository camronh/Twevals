...
def format_eval_list_table(eval_info_list: List[Dict[str, Any]]) -> Table:
    ...
    table = Table(title="Evaluation Functions", show_header=True, header_style="bold magenta", show_lines=True, expand=True)
    ...
    for info in eval_info_list:
        table.add_row(info['function'], info['dataset'], ", ".join(info['labels']),
                      fmt(info.get('input'), 60), fmt(info.get('reference'), 40), fmt(info.get('metadata'), 40))
    return table
