...
@eval(dataset="demo", evaluators=[check_length])
async def test_response(ctx: EvalContext):
    ctx.input = "Explain recursion"
...
@eval(
    dataset="qa",
    evaluators=[check_format, check_keywords, check_no_hallucination]
)
async def test_answer(ctx: EvalContext):
    ctx.input = "Why is the sky blue?"
    ctx.add_output(await my_agent(ctx.input))
```
