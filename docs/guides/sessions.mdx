...
<AccordionGroup>
  <Accordion title="Model Comparison">
    ...
    ```bash
    twevals evals/ --serve --session model-comparison --run-name gpt4-baseline
    twevals evals/ --serve --session model-comparison --run-name claude-3
    twevals evals/ --serve --session model-comparison --run-name gemini-pro
    ```
  ...
  <Accordion title="Iterative Development">
    ...
    ```bash
    twevals evals/ --serve --session bug-fix-123 --run-name initial
    # Make changes...
    twevals evals/ --serve --session bug-fix-123 --run-name attempt-2
    # More changes...
    twevals evals/ --serve --session bug-fix-123 --run-name fixed
    ```
  </Accordion>
  ...
  <Accordion title="CI/CD Integration">
    ...
    ```bash
    twevals evals/ --session "release-${VERSION}" --run-name "build-${BUILD_ID}" -o results.json
    ```
  </Accordion>
  ...
  <Accordion title="A/B Testing">
    ...
    ```bash
    twevals evals/ --serve --session prompt-experiment --run-name prompt-v1
    twevals evals/ --serve --session prompt-experiment --run-name prompt-v2-concise
    twevals evals/ --serve --session prompt-experiment --run-name prompt-v3-detailed
    ```
  </Accordion>
</AccordionGroup>
