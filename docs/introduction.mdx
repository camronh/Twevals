...
Twevals is a lightweight, code-first evaluation framework for testing AI agents and LLM applications. Inspired by Pytest and LangSmith, Twevals lets you
...
async def test_sentiment_analysis(ctx: EvalContext):
    ...
    # Evaluate!
    assert ctx.output == cxt.reference, f"Expected {ctx.reference}, got {ctx.output}"
```
...
Twevals spins up a local Web UI that makes it easy to filter, run, and rerun evals. Do deep analysis on the results

<img src="/assets/ui-screenshot.png" alt="Twevals Web UI" />
