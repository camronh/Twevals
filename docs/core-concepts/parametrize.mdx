...
async def test_sentiment(ctx: EvalContext, model, text, expected):
    ...
    ctx.add_output(await analyze_sentiment(text, model=model))
...
async def test_qa(ctx: EvalContext, question, answer):
    ...
    assert ctx.output == answer, f"Expected {answer}, got {ctx.output}"
```
...
async def test_intent(ctx: EvalContext, expected_intent, expected_sentiment):
    ...
    # expected_intent and expected_sentiment come from function args
    ...
    assert result["sentiment"] == expected_sentiment, \
        f"Expected sentiment {expected_sentiment}, got {result['sentiment']}"
```
