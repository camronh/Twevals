...
async def test_arithmetic(ctx: EvalContext):
    ctx.add_output(await calculator(ctx.input))
...
async def test_sentiment(ctx: EvalContext):
    ctx.add_output(await classify_sentiment(ctx.input))
...
async def test_comprehensive(ctx: EvalContext):
    ...
    # Different metrics with different keys
    ctx.add_score(
        "quantum" in ctx.output.lower(),
        "Mentions quantum",
        key="relevance"
    )
    ctx.add_score(
        len(ctx.output) < 500,
        f"Length: {len(ctx.output)}",
        key="brevity"
    )
    ctx.add_score(
        calculate_similarity(ctx.output, reference),
        "Semantic similarity",
        key="similarity"
    )
```
...
async def test_mixed(ctx: EvalContext):
    ctx.input = "Generate a summary"
    ctx.add_output(await summarizer(ctx.input))
...
@eval(dataset="api", evaluators=[check_format])
async def test_json_response(ctx: EvalContext):
    ctx.input = "Get user data"
    ctx.add_output(await api_call(ctx.input))
