...
Use the `serve` command to open an interactive web interface:
...
twevals run evals.py::test_greeting

# Filter by dataset
twevals run evals.py --dataset greetings
...
# Export to CSV
twevals run evals.py --csv results.csv

# Output compact JSON to stdout
twevals run evals.py --json
```
