...
<ParamField path="-f, --format" type="choice" default="json">
  Export format: `json`, `csv`, `pdf`, or `md`.
</ParamField>

```bash
ezvals export run.json -f pdf
...
```bash
ezvals export run.json -f pdf -o report.pdf
```
...
| `pdf` | PDF with stats chart and results table (requires `weasyprint`) |
| `md` | Markdown with ASCII bar charts and results table |

### PDF Export

PDF export requires the `weasyprint` library and system dependencies (Pango, Cairo):

```bash
# Install weasyprint
pip install ezvals[pdf]
...
ezvals export run.json -f pdf -o weekly-report.pdf
