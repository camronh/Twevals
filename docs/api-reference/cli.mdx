...
<ParamField path="-q, --quiet" type="flag">
  Reduce logging. Hides access logs from the server.
</ParamField>

```bash
twevals serve evals/ --quiet
```

## twevals run

Run evaluations headlessly and output results to the terminal.
...
```bash
twevals run evals/ --csv results.csv
```
...
```bash
twevals run evals/ --json
twevals run evals/ --json | jq '.results[0]'
```
...
```bash
twevals run evals/ -o results.json --csv results.csv
```

### Verbose Debug Run

```bash
twevals run evals/ -v --limit 5
```

### Production CI Pipeline

```bash
twevals run evals/ -c 16 --timeout 120 -o results.json --json
```
...
| `verbose` | boolean | Show stdout from evaluations | `run` |
| `results_dir` | string | Directory for results storage | `serve` |
| `port` | integer | Web UI server port | `serve` |
| `quiet` | boolean | Hide server access logs | `serve` |
