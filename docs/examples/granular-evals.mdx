...
Sometimes you can't parametrize. Each test case has unique logic—different ways of deriving ground truth, different validation. Twevals handles this naturally.
...
async def run_agent(ctx: EvalContext):
    ...
    response = await agent.run(ctx.input)
    return response
...
@eval(
    dataset="agent_tasks",
    input="Book a meeting with John tomorrow at 2pm",
    target=run_agent
)
async def test_calendar_booking(ctx: EvalContext):
    ...
    ctx.booking_details = john_booking  # Store for debugging
...
```bash
# Start the web UI
twevals serve evals/agent_tasks.py

# Run headlessly
twevals run evals/agent_tasks.py

# Run just one specific test
twevals run evals/agent_tasks.py::test_weather_lookup

# Run with verbose output
twevals run evals/agent_tasks.py --verbose
```
